...
public class PersonBean implements Serializable
{
   ...
   public void paginate()
   {
      ...
      TypedQuery<Person> query = this.entityManager.createQuery(criteria.select(root).where(getSearchPredicates(root)));
   ...
   private Predicate[] getSearchPredicates(Root<Person> root)
   {
      ...
      {
         predicatesList.add(builder.like(root.<String> get("vorname"), '%' + vorname + '%'));
      }
      String nachname = this.example.getNachname();
      if (nachname != null && !"".equals(nachname))
      {
         predicatesList.add(builder.like(root.<String> get("nachname"), '%' + nachname + '%'));
      }
      String strasse = this.example.getStrasse();
      if (strasse != null && !"".equals(strasse))
      {
         predicatesList.add(builder.like(root.<String> get("strasse"), '%' + strasse + '%'));
      }
      String plzOrt = this.example.getPlzOrt();
      if (plzOrt != null && !"".equals(plzOrt))
      {
         predicatesList.add(builder.like(root.<String> get("plzOrt"), '%' + plzOrt + '%'));
      }
   ...
   public List<Person> getAll()
   {
      ...
      CriteriaQuery<Person> criteria = this.entityManager.getCriteriaBuilder().createQuery(Person.class);
      return this.entityManager.createQuery(criteria.select(criteria.from(Person.class))).getResultList();
   }
